  ...
  describe '.verify' do
    ...
    context 'with expired token' do
      before do
        ...
        stub_google_token_verifier(id_token: valid_token, response_body: expired_response.to_json)
      end
    ...
    context 'with HTTP error response' do
      before do
        stub_google_token_verifier(id_token: valid_token, status: 500)
      end
    ...
    context 'with HTTP error response' do
      before do
        stub_google_token_verifier(id_token: invalid_token, status: 400, response_body: { error: 'invalid_token' }.to_json)
      end
    ...
    context 'when unexpected error occurs' do
      before do
        stub_google_token_verifier(id_token: valid_token, response_body: valid_token_response.to_json)
        allow(Rails.application.credentials).to receive(:gcp).and_return(nil)
      end
